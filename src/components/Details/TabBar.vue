<template>
	<div class="details-header-tabs">
		<tab-handle v-for="tab in shownTabs" :key="tab.name" :text="tab.text" :name="tab.name" :icon="tab.icon" :active="tab.name == activeTab" :short="shortTabs.includes(tab)" @tab-selected="$emit('tab-selected', $event)">{{tab.text}}</tab-handle>
	</div>
</template>
